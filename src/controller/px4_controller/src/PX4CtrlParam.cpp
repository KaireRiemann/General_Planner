...
#include <px4_controller/PX4CtrlParam.h>
...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle& nh)
{
  ...
  nh.param("auto_hover_on_start", auto_hover_on_start, false);
  ...
  if (takeoff_land.no_RC && !(takeoff_land.enable_auto_arm && takeoff_land.enable) && !auto_hover_on_start)
  {
    takeoff_land.no_RC = false;
    ROS_ERROR("\"no_RC\" requires either auto takeoff+arm or auto_hover_on_start enabled.");
  }
