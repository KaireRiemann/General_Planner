...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/perching_lock_topic", perching_lock_topic_, std::string("/perching/lock"));
    ...
    nh.param("fsm/perching_approach_velocity_alpha", perching_approach_velocity_alpha_, 0.35);
    ...
    nh.param("fsm/perching_terminal_thrust_range", perching_terminal_thrust_range_, 2.0);
    nh.param("fsm/perching_use_dynamics_terminal_accel", perching_use_dynamics_terminal_accel_, true);
    ...
    nh.param("fsm/perching_arrive_pos_thresh", perching_arrive_pos_thresh_, 0.20);
    nh.param("fsm/perching_arrive_vel_thresh", perching_arrive_vel_thresh_, 0.45);
    ...
    nh.param("fsm/perching_entry_tangent_vel_thresh", perching_entry_tangent_vel_thresh_, 0.25);
    nh.param("fsm/perching_commit_distance_thresh", perching_commit_distance_thresh_, 0.45);
    nh.param("fsm/perching_commit_rel_vel_thresh", perching_commit_rel_vel_thresh_, 0.25);
    nh.param("fsm/perching_commit_tangent_vel_thresh", perching_commit_tangent_vel_thresh_, 0.15);
    nh.param("fsm/perching_commit_normal_speed_min", perching_commit_normal_speed_min_, 0.03);
    nh.param("fsm/perching_commit_normal_speed_max", perching_commit_normal_speed_max_, 0.65);
    ...
    perching_entry_tangent_vel_thresh_ = std::max(0.0, perching_entry_tangent_vel_thresh_);
    ...
    perching_commit_tangent_vel_thresh_ = std::max(0.0, perching_commit_tangent_vel_thresh_);
    perching_commit_normal_speed_min_ = std::max(0.0, perching_commit_normal_speed_min_);
    perching_commit_normal_speed_max_ =
        std::max(perching_commit_normal_speed_min_ + 1.0e-3,
                 perching_commit_normal_speed_max_);
    ...
    task_runtime_stage_ = initialTaskRuntimeStage();
    ...
    if (use_perching_task_)
    {
      ...
      perching_trigger_sub_ = nh.subscribe<std_msgs::Empty>(
          ...
          this);
      ROS_INFO("Perching task enabled. target_odom_topic=%s trigger_topic=%s auto_start=%s robot_l=%.3f v_plus=%.3f approach_vel_alpha=%.2f dyn_terminal_acc=%s thrust_nom=%.2f thrust_range=%.2f entry_tangent=%.2f commit_dist=%.2f commit_rel_vel=%.2f commit_tangent=%.2f commit_normal=[%.2f %.2f] commit_hold=%.2f",
               ...
               perching_commit_tangent_vel_thresh_,
               perching_commit_normal_speed_min_,
               perching_commit_normal_speed_max_,
      ...
      perching_lock_pub_.publish(unlock_msg);
    }
    if (!use_tracking_task_ && !use_perching_task_ && target_type_ == TARGET_TYPE::MANUAL_TARGET)
    {
      waypoint_sub_ = nh.subscribe("/goal", 1, &EGOReplanFSM::waypointCallback, this);
    }
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case WAIT_TARGET:
    {
      if (integratedTrackingPerchingMode())
      {
        ...
        if (task_runtime_stage_ == TaskRuntimeStage::PERCHING_CONTACT && !perching_round_active_)
        {
          if (!shouldReleasePerchingContactHold())
          {
            goto force_return;
          }
    ...
    case EXEC_TRAJ:
    {
      ...
      const bool tracking_active =
          use_tracking_task_ &&
          have_tracking_ref_ &&
          task_runtime_stage_ == TaskRuntimeStage::TRACKING_FOLLOW;
      const bool perching_runtime =
          use_perching_task_ &&
          (!use_tracking_task_ || task_runtime_stage_ != TaskRuntimeStage::TRACKING_FOLLOW);
      ...
      if (tracking_active && integratedTrackingPerchingMode())
      {
        ...
        if (shouldEnterPerchingApproach(entry_terminal))
        {
          ...
          transitionTaskRuntimeStage(TaskRuntimeStage::PERCHING_APPROACH,
                                     ...
                                     &entry_terminal);
          changeFSMExecState(REPLAN_TRAJ, "PERCHING_STAGE_ENTRY");
          break;
        }
      }
      ...
      if (perching_runtime)
      {
        ...
        if (perching_commit_active && frozenPerchingTerminalReady())
        {
          execution_terminal = frozen_perching_terminal_;
          have_execution_terminal = true;
        }
        else if (have_active_perching_terminal_ && active_perching_terminal_.valid)
        {
          execution_terminal = active_perching_terminal_;
          have_execution_terminal = true;
        }
        else if (perching_target_provider_ &&
                 perching_target_provider_->buildTerminalState(odom_pos_,
                                                               planner_manager_->pp_.max_vel_,
                                                               execution_terminal))
        {
          have_execution_terminal = execution_terminal.valid;
        }
        ...
        const bool have_live_terminal =
            ...
            perching_target_provider_->buildTerminalStateAtPrediction(0.0, live_terminal);
        const Eigen::Vector3d active_contact =
            perching_commit_active ? committed_tail_contact
                                   : (have_execution_terminal ? execution_terminal.terminal_position
                                    : (have_live_terminal ? live_terminal.terminal_position
                                                         : planned_contact));
        const Eigen::Vector3d active_contact_vel =
            perching_commit_active ? committed_tail_velocity
                                   : (have_execution_terminal ? execution_terminal.terminal_velocity
                                    : (have_live_terminal ? live_terminal.terminal_velocity
                                                         : Eigen::Vector3d::Zero()));
        const double dist_to_contact = (active_contact - odom_pos_).norm();
        const double rel_contact_speed = (odom_vel_ - active_contact_vel).norm();
        ...
        const bool terminal_envelope_reached =
            ...
            dist_to_contact <= (perching_commit_active
                                    ? std::max(0.08, 0.75 * perching_arrive_pos_thresh_)
                                    : perching_arrive_pos_thresh_) &&
            rel_contact_speed <= (perching_commit_active
                                      ? std::max(0.10, 0.75 * perching_arrive_vel_thresh_)
                                      : perching_arrive_vel_thresh_);
        ...
        if (perching_approach_active && (commit_gate || terminal_envelope_reached))
        {
          ...
          transitionTaskRuntimeStage(TaskRuntimeStage::PERCHING_COMMIT,
                                     ...
                                     frozenPerchingTerminalReady() ? &frozen_perching_terminal_
                                                                   : (have_commit_candidate ? &commit_candidate_terminal
                                                                                            : &execution_terminal));
          changeFSMExecState(REPLAN_TRAJ, "PERCHING_COMMIT_ENTRY");
          break;
        }
        ...
        {
          ROS_INFO("[FSM] perching successor replan (approach): remaining_t=%.2f", remaining_time);
          changeFSMExecState(REPLAN_TRAJ, "PERCHING_SUCCESSOR");
          break;
        }
        if (perching_commit_active &&
            ...
            remaining_time <= successor_trigger_time)
        {
          logPerchingCommitSuppressedReplan("frozen_commit_reference",
                                            remaining_time,
                                            dist_to_contact,
                                            rel_contact_speed);
        }
        ...
        if (perching_replan_if_unsafe_ &&
            replan_allowed &&
            remaining_time > 0.8 &&
            dist_to_contact > std::max(0.55, 1.2 * perching_arrive_pos_thresh_) &&
            !currentTrajStillUsable(std::min(state2state_keep_lookahead_, remaining_time)))
        {
          ROS_WARN("[FSM] perching trajectory became unsafe during %s, replan current landing round.",
                   taskRuntimeStageString(task_runtime_stage_));
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    if (new_state == WAIT_TARGET || new_state == EMERGENCY_STOP)
    {
      ...
      const Eigen::Vector3d preserved_frozen_surface_tangent_x = frozen_surface_tangent_x_;
      const Eigen::Vector3d preserved_frozen_surface_tangent_y = frozen_surface_tangent_y_;
      const Eigen::Vector3d preserved_frozen_surface_normal = frozen_surface_normal_;
      resetPlannedTaskTargets();
      if (preserve_perching_contact_hold)
      {
        have_frozen_perching_terminal_ = preserved_have_frozen_terminal;
        frozen_perching_terminal_ = preserved_frozen_terminal;
        frozen_perching_reference_time_ = preserved_frozen_reference_time;
        frozen_surface_tangent_x_ = preserved_frozen_surface_tangent_x;
        frozen_surface_tangent_y_ = preserved_frozen_surface_tangent_y;
        frozen_surface_normal_ = preserved_frozen_surface_normal;
        transitionTaskRuntimeStage(TaskRuntimeStage::PERCHING_CONTACT,
                                   "preserve_contact_hold",
                                   preserved_have_frozen_terminal ? &preserved_frozen_terminal
                                                                  : nullptr);
        perching_contact_latched_ = true;
      }
    }
  ...
  void EGOReplanFSM::printFSMExecState()
  {
    ...
    const bool tracking_stage_selected =
        use_tracking_task_ &&
        (!use_perching_task_ || task_runtime_stage_ == TaskRuntimeStage::TRACKING_FOLLOW);
    const bool perching_stage_selected =
        use_perching_task_ &&
        (!use_tracking_task_ || task_runtime_stage_ != TaskRuntimeStage::TRACKING_FOLLOW);
    const bool task_has_target =
        tracking_stage_selected ? have_tracking_ref_
                                : (perching_stage_selected ? have_perching_target_odom_ : have_target_);
    const bool task_has_trigger =
        tracking_stage_selected ? true
                                : (integratedTrackingPerchingMode() ? true : have_trigger_);
    if (!have_odom_ || !task_has_target || !task_has_trigger || (planner_manager_->pp_.drone_id >= 1 && !have_recv_pre_agent_))
      std::cout << ". Waiting for ";
    if (!have_odom_) std::cout << "odom,";
    if (!task_has_target) std::cout << "target,";
    if (!task_has_trigger) std::cout << "trigger,";
    if (integratedTrackingPerchingMode())
    {
      std::cout << " stage=" << taskRuntimeStageString(task_runtime_stage_);
    }
 ...
 void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
  ...
  bool EGOReplanFSM::integratedTrackingPerchingMode() const
  {
    return use_tracking_task_ && use_perching_task_;
  }

  bool EGOReplanFSM::standalonePerchingMode() const
  {
    return use_perching_task_ && !use_tracking_task_;
  }

  EGOReplanFSM::TaskRuntimeStage EGOReplanFSM::initialTaskRuntimeStage() const
  {
    ...
    if (use_tracking_task_)
    {
      return TaskRuntimeStage::TRACKING_FOLLOW;
    }
    return TaskRuntimeStage::NONE;
  }

  const char *EGOReplanFSM::taskRuntimeStageString(const TaskRuntimeStage stage) const
  {
    switch (stage)
    {
    case TaskRuntimeStage::TRACKING_FOLLOW:
      return "tracking_follow";
    case TaskRuntimeStage::PERCHING_APPROACH:
      return "perching_approach";
    case TaskRuntimeStage::PERCHING_COMMIT:
      return "perching_commit";
    case TaskRuntimeStage::PERCHING_CONTACT:
      return "perching_contact";
    case TaskRuntimeStage::NONE:
    default:
      return "none";
    }
  }

  bool EGOReplanFSM::buildPerchingPlanningTerminal(const Eigen::Vector3d &planning_start,
                                                   runtime::PerchingTerminalState &terminal,
                                                   const bool apply_map_fallback) const
  {
    terminal = runtime::PerchingTerminalState{};
    if (!use_perching_task_ || !have_perching_target_odom_ || !perching_target_provider_)
    {
      return false;
    }

    if (!perching_target_provider_->buildTerminalState(planning_start,
                                                       planner_manager_->pp_.max_vel_,
                                                       terminal))
    {
      return false;
    ...
    if (!apply_map_fallback || planner_manager_ == nullptr || planner_manager_->grid_map_ == nullptr)
    {
      return true;
    }

    const auto inUpdatedMapWindow = [&](const Eigen::Vector3d &pt) -> bool
    {
      const Eigen::Vector3d low = planner_manager_->grid_map_->getUpdatedBoxLow();
      const Eigen::Vector3d high = planner_manager_->grid_map_->getUpdatedBoxHigh();
      if (!low.allFinite() || !high.allFinite())
      {
        return true;
      }
      const double margin =
          std::max(0.15, 1.5 * planner_manager_->grid_map_->getResolution());
      return (pt.array() >= (low.array() + margin)).all() &&
             (pt.array() <= (high.array() - margin)).all();
    };

    if (inUpdatedMapWindow(terminal.terminal_position))
    {
      return true;
    }

    const std::array<double, 5> fallback_scales{{0.75, 0.55, 0.35, 0.20, 0.0}};
    for (const double scale : fallback_scales)
    {
      runtime::PerchingTerminalState candidate;
      const double prediction = std::max(0.0, terminal.prediction_time * scale);
      if (!perching_target_provider_->buildTerminalStateAtPrediction(prediction, candidate))
      {
        continue;
      }
      if (inUpdatedMapWindow(candidate.terminal_position))
      {
        terminal = candidate;
        ROS_WARN("[FSM] perching terminal outside map, fallback prediction_t=%.2f",
  ...
  bool EGOReplanFSM::perchingEntryGateSatisfied(const runtime::PerchingTerminalState &terminal,
                                                ...
                                                double *relative_anchor_speed) const
  {
    ...
    const Eigen::Vector3d rel_surface_velocity =
        odom_vel_ - terminal.plate_velocity;
    const double rel_tangent_speed =
        std::hypot(rel_surface_velocity.dot(terminal.landing_tangent_x),
                   rel_surface_velocity.dot(terminal.landing_tangent_y));
    if (distance_to_anchor != nullptr)
    {
    ...
    if (relative_anchor_speed != nullptr)
    {
      *relative_anchor_speed = rel_tangent_speed;
    }

    const double dist_to_contact =
        (terminal.terminal_position - odom_pos_).norm();
    // Fast-Perching starts solving directly from the current relative state
    // instead of waiting for a separate tracking anchor to be hit exactly.
    // Our integrated handoff should therefore open once tracking has stabilized
    // the relative motion inside a broader contact-envelope capture window,
    // while commit remains the strict frozen-reference stage near the true
    // approach anchor.
    const double anchor_window =
        std::max(0.35, 1.25 * perching_arrive_pos_thresh_);
  ...
  bool EGOReplanFSM::shouldEnterPerchingApproach(runtime::PerchingTerminalState &terminal)
  {
    ...
    double dist_to_anchor = 0.0;
    double rel_tangent_speed = 0.0;
    const double dist_to_contact =
        (terminal.terminal_position - odom_pos_).norm();
    const Eigen::Vector3d rel_surface_velocity =
        odom_vel_ - terminal.plate_velocity;
    const double rel_normal_speed =
        rel_surface_velocity.dot(terminal.landing_normal);
    const double contact_capture_window =
        std::max(std::max(0.90, terminal.approach_distance + 0.35),
                 2.0 * perching_arrive_pos_thresh_);
    const double prediction_horizon =
    ...
    const bool gate_ok =
        perchingEntryGateSatisfied(terminal, &dist_to_anchor, &rel_tangent_speed);
    ...
    if (perching_stage_entry_stable_since_ < 0.0)
    {
      perching_stage_entry_stable_since_ = now;
    }
                                               ...
                                               double *relative_anchor_speed)
  {
    ...
    const double rel_tangent_speed =
        std::hypot(rel_surface_velocity.dot(terminal.landing_tangent_x),
                   rel_surface_velocity.dot(terminal.landing_tangent_y));
    const double normal_closing_speed =
        -rel_surface_velocity.dot(terminal.landing_normal);
    if (distance_to_anchor != nullptr)
    {
      *distance_to_anchor = dist_to_anchor;
    ...
    const bool inside_commit_window =
        ...
        rel_tangent_speed <= perching_commit_tangent_vel_thresh_ &&
        normal_closing_speed >= perching_commit_normal_speed_min_ &&
        normal_closing_speed <= perching_commit_normal_speed_max_;
    ...
    if (!inside_commit_window)
    {
      perching_commit_stable_since_ = -1.0;
      ROS_INFO_THROTTLE(0.8,
                        "[FSM] perching commit gate pending: dist_to_anchor=%.2f/%.2f rel_anchor_vel=%.2f/%.2f rel_tangent_vel=%.2f/%.2f normal_closing=%.2f/[%.2f %.2f] remaining_t=%.2f stage=%s",
                        dist_to_anchor,
                        perching_commit_distance_thresh_,
                        rel_anchor_speed,
                        perching_commit_rel_vel_thresh_,
    ...
    ROS_INFO_THROTTLE(0.8,
                      "[FSM] perching commit gate stable: dist_to_anchor=%.2f/%.2f rel_anchor_vel=%.2f/%.2f rel_tangent_vel=%.2f/%.2f normal_closing=%.2f/[%.2f %.2f] remaining_t=%.2f hold=%.2f/%.2f",
                      dist_to_anchor,
                      perching_commit_distance_thresh_,
                      rel_anchor_speed,
                      perching_commit_rel_vel_thresh_,
    ...
    ROS_INFO("[FSM] enter perching_commit: reason=%s dist_to_anchor=%.2f rel_anchor_vel=%.2f rel_tangent_vel=%.2f normal_closing=%.2f hold=%.2f ref_t=%.2f",
  ...
  {
    return task_runtime_stage_ == TaskRuntimeStage::PERCHING_COMMIT ||
           task_runtime_stage_ == TaskRuntimeStage::PERCHING_CONTACT;
  }

  bool EGOReplanFSM::frozenPerchingTerminalReady() const
  {
    return have_frozen_perching_terminal_ && frozen_perching_terminal_.valid;
  }

  void EGOReplanFSM::transitionTaskRuntimeStage(const TaskRuntimeStage new_stage,
                                                const char *reason,
                                                const runtime::PerchingTerminalState *terminal)
  {
    if (task_runtime_stage_ == new_stage)
    {
      return;
    }

    const TaskRuntimeStage previous_stage = task_runtime_stage_;
    task_runtime_stage_ = new_stage;

    if (terminal != nullptr && terminal->valid)
    {
      ROS_INFO("[FSM] task runtime stage: %s -> %s reason=%s ref_t=%.2f approach=[%.2f %.2f %.2f] contact=[%.2f %.2f %.2f] frozen_commit=%s",
               taskRuntimeStageString(previous_stage),
               taskRuntimeStageString(new_stage),
               ...
               frozenPerchingTerminalReady() ? "yes" : "no");
      return;
    }

    ROS_INFO("[FSM] task runtime stage: %s -> %s reason=%s frozen_commit=%s",
             taskRuntimeStageString(previous_stage),
             taskRuntimeStageString(new_stage),
             reason != nullptr ? reason : "none",
             frozenPerchingTerminalReady() ? "yes" : "no");
  ...
  void EGOReplanFSM::logPerchingCommitSuppressedReplan(const char *reason,
                                                       double remaining_time,
                                                       double dist_to_contact,
                                                       double rel_contact_speed) const
  {
    ROS_INFO_THROTTLE(0.8,
                      "[FSM] suppress generic successor replan in %s: reason=%s remaining_t=%.2f dist_to_contact=%.2f rel_contact_vel=%.2f frozen_ref_t=%.2f",
                      taskRuntimeStageString(task_runtime_stage_),
                      reason != nullptr ? reason : "frozen_commit",
                      remaining_time,
                      dist_to_contact,
                      rel_contact_speed,
                                                     ...
                                                     const char *reason)
  {
    ...
    frozen_perching_wall_time_ = ros::Time::now().toSec();
    ...
    ROS_INFO("[FSM] freeze perching commit reference (%s): ref_t=%.2f approach=[%.2f %.2f %.2f] contact=[%.2f %.2f %.2f] prev_acc=[%.2f %.2f %.2f] committed_acc=[%.2f %.2f %.2f] tx=[%.2f %.2f %.2f] ty=[%.2f %.2f %.2f] normal=[%.2f %.2f %.2f]",
             ...
             frozen_perching_terminal_.approach_anchor_position.y(),
             frozen_perching_terminal_.approach_anchor_position.z(),
             ...
             frozen_surface_tangent_y_.x(),
             frozen_surface_tangent_y_.y(),
             frozen_surface_tangent_y_.z(),
             frozen_surface_normal_.x(),
             frozen_surface_normal_.y(),
  ...
  void EGOReplanFSM::clearFrozenPerchingTerminalReference()
  {
    ...
    frozen_perching_wall_time_ = -1.0;
    perching_commit_stable_since_ = -1.0;
  ...
  bool EGOReplanFSM::shouldReleasePerchingContactHold() const
  {
    if (!integratedTrackingPerchingMode() || task_runtime_stage_ != TaskRuntimeStage::PERCHING_CONTACT)
    {
      return true;
    }

    if (perching_contact_latched_ && !perching_triggered_)
    {
      return false;
    }

    runtime::PerchingTerminalState current_terminal;
    if (perching_target_provider_ &&
        perching_target_provider_->buildTerminalStateAtPrediction(0.0, current_terminal))
    {
      // Contact hold release follows the current plate state so that tracking
      // can resume only after the live plate has moved away from the frozen
      // commit/contact neighborhood.
    }
    else if (have_frozen_perching_terminal_ && frozen_perching_terminal_.valid)
    {
      current_terminal = frozen_perching_terminal_;
    }
    else if (have_active_perching_terminal_ && active_perching_terminal_.valid)
    {
      current_terminal = active_perching_terminal_;
    }
    else
    {
      return true;
    ...
    const double release_distance =
        std::max(0.80, 2.0 * perching_arrive_pos_thresh_);
    const double dist_to_contact =
        (current_terminal.terminal_position - odom_pos_).norm();
    const double rel_contact_speed =
        (odom_vel_ - current_terminal.terminal_velocity).norm();
    return dist_to_contact > release_distance ||
           rel_contact_speed > std::max(0.40, 1.25 * perching_arrive_vel_thresh_);
  }
                                         ...
                                         bool flag_randomPolyTraj)
  {
    ...
    if (tracking_stage_selected && !tracking_active)
    {
      ROS_WARN_THROTTLE(1.0,
                        "[FSM] tracking planning aborted: tracking_follow selected but reference is unavailable.");
      return false;
    }
    if (perching_stage_selected && !perching_ready)
    {
      ROS_WARN_THROTTLE(1.0,
                        "[FSM] perching planning aborted: %s selected but terminal semantics are unavailable.",
                        taskRuntimeStageString(task_runtime_stage_));
      return false;
    }
    ...
    else if (perching_active)
    {
      ...
      if (use_frozen_commit_reference)
      {
        ...
        if (frozen_perching_wall_time_ >= 0.0)
        {
          const double elapsed_since_freeze =
              std::max(0.0, ros::Time::now().toSec() - frozen_perching_wall_time_);
          const double aged_reference_time =
              perching_terminal.prediction_time - elapsed_since_freeze;
          if (aged_reference_time >= 0.0)
          {
            perching_terminal.prediction_time = aged_reference_time;
          }
          else
          {
            perching_terminal.plate_position +=
                perching_terminal.plate_velocity * (-aged_reference_time);
            perching_terminal.plate_position_now = perching_terminal.plate_position;
            perching_terminal.prediction_time = 0.0;
          }
          perching_terminal.terminal_position =
              perching_terminal.plate_position +
              perching_robot_l_ * perching_terminal.landing_normal;
          perching_terminal.terminal_velocity =
              perching_terminal.plate_velocity -
              perching_v_plus_ * perching_terminal.landing_normal;
          if (perching_terminal.use_dynamics_terminal_accel)
          {
          ...
          {
            perching_terminal.terminal_acceleration = Eigen::Vector3d::Zero();
          }
          perching_terminal.approach_anchor_position =
              perching_terminal.terminal_position -
              perching_terminal.approach_distance *
                  perching_terminal.landing_normal;
          perching_terminal.approach_anchor_velocity =
              perching_terminal.plate_velocity -
              perching_approach_velocity_alpha_ *
                  perching_v_plus_ * perching_terminal.landing_normal;
          perching_terminal.approach_anchor_acceleration = Eigen::Vector3d::Zero();
        }
      }
      else if (!buildPerchingPlanningTerminal(start_pt_, perching_terminal, true))
      {
        ROS_WARN("[FSM] perching planning aborted: unable to build %s terminal bundle.",
                 taskRuntimeStageString(task_runtime_stage_));
        return false;
      }

      local_target_pt_ = perching_terminal.approach_anchor_position;
      local_target_vel_ = perching_terminal.approach_anchor_velocity;
      ...
      ROS_INFO("[FSM] perching terminal source=%s stage=%s frozen=%s prediction_t=%.2f approach=[%.2f %.2f %.2f] contact=[%.2f %.2f %.2f] vel=[%.2f %.2f %.2f] acc=[%.2f %.2f %.2f] normal=[%.2f %.2f %.2f]",
               ...
               final_goal_.y(),
               final_goal_.z(),
               perching_terminal.terminal_velocity.x(),
               perching_terminal.terminal_velocity.y(),
               perching_terminal.terminal_velocity.z(),
    ...
    else
    {
      ...
      {
        task_prefer_corridor = true;
        resolved_space_pref = perching_active ? "auto->corridor(perching_safe)" : "auto->corridor";
      }
      else if (planner_manager_->esdfModeEnabled())
      {
        task_prefer_esdf = true;
        resolved_space_pref = perching_active ? "auto->esdf(perching_safe)" : "auto->esdf";
      }
      else
      {
        task_force_plain = true;
        resolved_space_pref = perching_active ? "auto->plain(perching_no_safe_map)" : "auto->plain";
      }
    }
    ...
    else if (perching_active)
    {
      task_definition = tasks::TaskFactory::makePerchingDefinition(
          ...
          perching_terminal.approach_anchor_position,
          perching_terminal.approach_anchor_velocity,
          perching_terminal.approach_anchor_acceleration,
          perching_terminal.terminal_position,
          perching_terminal.terminal_velocity,
          perching_terminal.terminal_acceleration,
          perching_terminal.plate_position,
          ...
          perching_terminal.landing_normal,
          perching_terminal.tangential_velocity_seed,
          perching_terminal.thrust_phase_seed,
          perching_robot_l_,
          perching_v_plus_,
          perching_terminal.terminal_thrust_nominal,
    ...
    if (plan_success)
    {
      ...
      else if (perching_active)
      {
        ...
        {
          Eigen::VectorXd solved_extra_vars;
          if (auto *optimizer = planner_manager_->getOptimizer();
              optimizer != nullptr &&
              optimizer->getLastPerchingExtraVariables(solved_extra_vars) &&
              solved_extra_vars.size() >= 3)
          {
            const Eigen::Vector2d nu_seed = solved_extra_vars.head<2>();
            const double tau_f_seed = solved_extra_vars(2);
            perching_target_provider_->setTerminalWarmStartHint(nu_seed, tau_f_seed);
            ROS_INFO("[FSM] cached perching warm-start seed nu=[%.3f %.3f] tau_f=%.3f",
                     nu_seed.x(),
                     nu_seed.y(),
                     tau_f_seed);
          }
        ...
        active_perching_terminal_ = perching_terminal;
        have_active_perching_terminal_ = perching_terminal.valid;
        perching_contact_latched_ = false;
        perching_triggered_ = false;
        have_trigger_ = false;
        ROS_INFO("[FSM] perching stage=%s accepted with frozen_commit=%s. Trigger consumed for current round.",
                 taskRuntimeStageString(task_runtime_stage_),
                 have_frozen_perching_terminal_ ? "yes" : "no");
      }
      else
      {
