...
namespace ego_planner
{
  namespace
  {
    ...
    const char *spaceModelPreferenceString(const core::SpaceModelPreference pref)
    {
      switch (pref)
      {
      case core::SpaceModelPreference::PLAIN:
        return "PLAIN";
      case core::SpaceModelPreference::ESDF:
        return "ESDF";
      case core::SpaceModelPreference::CORRIDOR:
        return "CORRIDOR";
      case core::SpaceModelPreference::VISIBLE_REGION:
        return "VISIBLE_REGION";
      case core::SpaceModelPreference::TERMINAL_MANIFOLD:
        return "TERMINAL_MANIFOLD";
      case core::SpaceModelPreference::AUTO:
      default:
        return "AUTO";
      }
    }

    const char *seedKindString(const core::SeedSpec::Kind kind)
    {
      switch (kind)
      {
      case core::SeedSpec::Kind::PLAIN_INIT:
        return "PLAIN_INIT";
      case core::SeedSpec::Kind::ESDF_INIT:
        return "ESDF_INIT";
      case core::SeedSpec::Kind::GUIDE_PATH_INIT:
        return "GUIDE_PATH_INIT";
      case core::SeedSpec::Kind::CORRIDOR_INIT:
        return "CORRIDOR_INIT";
      case core::SeedSpec::Kind::SEMANTIC_INIT:
        return "SEMANTIC_INIT";
      case core::SeedSpec::Kind::WARM_START:
        return "WARM_START";
      case core::SeedSpec::Kind::NONE:
      default:
        return "NONE";
      }
    }

    int countEnabledFeasibleSets(const core::PlanningProblem &problem,
                                 const core::FeasibleSetType type)
    {
      int count = 0;
                                                           ...
                                                           core::PlanningSolution &solution)
  {
    ...
    if (!state_to_state_initializer_)
    {
      return fillCompiledFailure("state-to-state initializer is not available");
    }
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    solver::StateToStateInitResources init_resources;
    init_resources.plan_params = &pp_;
    init_resources.traj_container = &traj_;
    init_resources.continuous_failures_count = &continous_failures_count_;
    init_resources.grid_map = grid_map_;
    init_resources.jps_astar = jps_astar_.get();
    init_resources.optimizer = ploy_traj_opt_.get();
    init_resources.guide_min_clearance = guide_min_clearance_;
    init_resources.guide_sparse_min_inner = guide_sparse_min_inner_;
    init_resources.guide_sparse_max_inner = guide_sparse_max_inner_;
    init_resources.guide_turn_angle_deg = guide_turn_angle_deg_;
    init_resources.sfc_progress = sfc_progress_;
    init_resources.sfc_range = sfc_range_;
    if (!state_to_state_initializer_)
    {
      state_to_state_initializer_.reset(new solver::StateToStateInitializer(init_resources));
    }
    else
    {
      state_to_state_initializer_->setResources(init_resources);
    }
  }
